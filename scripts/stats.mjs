...
const perBoard = await M.aggregate([
  { $group: { _id: "$deal", total: { $sum: 1 }, live: { $sum: { $cond: [{ $ifNull: ["$stoppedAt", false] }, 0, 1] } } } },
  { $sort: { live: -1 } },
]).toArray();
console.log("boards: %s", perBoard.map((b) => `${b._id}=${b.live}`).join("  "));
