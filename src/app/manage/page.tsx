...
import { dealOf } from "@/lib/passes";
import Pass, { PASS_STATUS } from "@/models/Pass";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "My codes - Claude Coupons",
  robots: { index: false },
};
...
export default async function ManagePage() {
  ...
  return (
    <section className="mx-auto mt-8 max-w-2xl">
      <h1 className="text-[28px] font-bold">My codes</h1>
      <p className="mt-1">
      ...
      {listings.length === 0 ? (
        ...
        <>
            ...
            <table className="w-full border-collapse text-[15px]">
              <thead>
                <tr className="text-xs tracking-wide text-muted uppercase">
                  {["Code", "Board", "Status", "Unlocked", "Claimed", "Dead reports"].map((h) => (
              ...
              <tbody>
                {listings.map((pass) => (
                  <tr key={pass._id.toString()}>
                    ...
                    <td className="border-b border-line px-2.5 py-2">{dealOf(pass).name}</td>
          ...
          <p className="mt-4 text-sm text-muted">
            A listing hides itself once it has been unlocked as many times as that program&rsquo;s
            code is good for, after repeated &ldquo;didn&rsquo;t work&rdquo; reports, or 21 days
            on the board. Still have uses left on one that stopped showing?{" "}
            <a className="text-accent-dark underline" href="/submit">
              Paste it in again
            </a>{" "}
            and it goes back up.
