    ...
    <html lang="en">
      <body className="antialiased">
        <header className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-5 py-5">
          <Link
            href="/"
            className="flex items-center gap-2 text-[19px] font-bold no-underline"
          >
          ...
          <nav className="flex items-center gap-4 text-[15px]">
            {/* Sitewide link, so the guest pass guide is reachable from every page and
                carries the internal link weight for the phrase it targets. */}
            ...
            {user ? (
              <>
                ...
                >
                  <button
                    type="submit"
                    className="cursor-pointer hover:text-accent-dark"
                  >
                    Sign out
                  </button>
                </form>
        ...
        <footer className="mx-auto max-w-5xl border-t border-line px-5 pt-6 pb-10 text-[13px] text-muted">
          <p>
            ...
            . Pass links are personal referral links; whoever shared one
            (including us) may earn usage credits from Anthropic if you later
            subscribe. Claiming a pass requires a payment card and the account
            converts to a paid Claude Pro subscription after 7 days unless
            canceled.
          </p>
          <p className="mt-2">
